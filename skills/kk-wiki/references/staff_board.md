...
- 2026-09-25 실측: 최신 글 18팀 = 텍스트 표 15팀 + 이미지 게시글 3팀(가치혁신·총무복지·국제협력 → OCR 보정 덤프) → **18/18 표**. 처음엔 헤더 규칙이 좁아(직무×담당만, 첫 행만) 학연운영팀(대·중·소분류)·수탁사업운영팀(헤더 5번째 행)·구매·자산팀(담당업무)이 "본문 없음"으로 빠졌었다 → 갱신 때 `status` 의 "이미지·본문만" 수를 반드시 보고, 남은 팀은 글을 직접 열어 표인지 이미지인지 확인한다.
