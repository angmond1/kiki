...
5. **답 제시(형식 고정, 2026-09-25 사용자 요청 "일목요연·시인성")** — 순서(2026-09-26 사용자 교정 "물어본 것부터"): ① **물어본 것에 대한 답**을 질문 순서대로 맨 위에, 각각 표 하나 또는 한두 줄(예: "필요 서류 + 결재선" 질문이면 서류 표 → 결재선 한 줄이 첫 화면). 절차·배경 설명으로 시작하지 않는다 → ② **꼭 필요한 부가사항**(절차·기한·작성 요령·상황별 처리 — 그 건에 실제로 걸리는 것만, 문장 끝 출처 번호 `[n]`) → ③ **담당자**(아래) → ④ **⚠️ 상충 / 위키에 없는 것 / 확인처** → ⑤ 출처 `[n] 경로 (수정일) 링크` 목록. 수치 비교가 필요한 질문(한도·단가)만 ①을 "구분 | 수치 | 비고" 표로.
