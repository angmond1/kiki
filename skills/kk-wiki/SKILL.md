...
5. **답 제시(형식 고정, 2026-09-25 사용자 요청 "일목요연·시인성")** — 순서: ① **한눈에 표**(구분 | 핵심 수치 | 비고, 3~4행) → ② 구분별 절(국내/해외 등) 짧은 표 + 규칙 불릿(문장 끝에 출처 번호 `[n]`) → ③ **상황별 처리 표**(상황 | 처리 | 출처) — 공제·반납·예외처럼 "내가 뭘 해야 하나"가 한 줄에 보이게 → ④ **위키에 없는 것 / 확인처**(담당팀·연락처) → ⑤ 출처 `[n] 경로 (수정일) 링크` 목록. 숫자는 표에만, 문장은 짧게, 원문 인용은 뜻이 갈릴 수 있는 조항에만 따옴표로. 서로 어긋나는 페이지는 둘 다 + 최근 것 표시. 없는 내용은 지어내지 않는다.
