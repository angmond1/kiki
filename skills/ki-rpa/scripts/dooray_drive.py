# -*- coding: utf-8 -*-
"""ki-rpa 코어 (2) — dooray drive 폴더 검색·구조파악·업로드·처리완료 아카이브.

인증: dooray 개인 토큰 (업로드는 세션쿠키로 안 됨 → 토큰 필요).
  토큰 로드 우선순위: 환경변수 DOORAY_TOKEN → ~/.claude/kiki/ki-rpa.env (DOORAY_TOKEN=...)
...
def _load_token() -> str:
    ...
    if t:
        return t
    p = os.path.expanduser("~/.claude/kiki/ki-rpa.env")
    if os.path.exists(p):
    ...
    raise RuntimeError(
        "dooray 토큰이 필요합니다. ~/.claude/kiki/ki-rpa.env 에 'DOORAY_TOKEN=...' 저장 "
