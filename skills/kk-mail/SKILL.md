...
- **제목으로도 거르기**(사용자가 요청할 때만 — 발신 주소를 자주 바꾸는 predatory 저널·학회 등): 구절은 위 '제목 키워드 고르기' 대로 고른다(예: `Journal of ○○ Science` — 광고 문구·인사말 제외). 지난 메일은 그 구절로 `previewSubjectRule` → 표 → confirm → `reportSpam`. 앞으로 올 메일까지 거르려면 그 구절로 자동분류 규칙을 만들되 대상을 `toFolderName: 'spam'`(스팸메일함)으로 한다 — 시스템 폴더를 대상으로 한 규칙은 아직 실측 전이라 첫 사용 때 confirm 후 1건으로 확인하고, 거부되면 사용자 폴더(예: 광고)로 대신하자고 제안한다.
