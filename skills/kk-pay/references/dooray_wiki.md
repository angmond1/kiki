...
## 규정 변경 감지 (반년마다 권장)
- 공식 API 로 트리 전체 `pages` 를 walk 하며 각 페이지 `updatedAt`·본문을 수집하고 이전 스냅샷과 비교(신규/삭제/본문변경/최근수정). kiki 가 의존하는 핵심 페이지는 **「2. 지급신청 매뉴얼」·「7. RPA 지급신청 안내」·구매·자산팀·인사경영팀(출장·전문가활용)** — 이들의 `updatedAt` 이 kiki 스냅샷 날짜보다 뒤면 diff 후 반영. (2026-09-18 실행: 매뉴얼 09-15 개정 등 반영, `docs/HISTORY.md` 참조)
