...
> 비목/집행기준은 KIST 규정이라 개정됨. `bimok_reference.md` 로 1차 판단하되, **애매하거나 최신 확인이 필요하면 여기서 실시간 검색**(권위·최신).
...
## 검색법
- **공식 API** (개인 토큰): `~/.claude/kiki/k-rpa.env` 의 토큰으로
...
- 결과로 `bimok_reference.md` 의 1차 판단을 검증/보강. 최종 비목 확정은 사용자·행정원.
