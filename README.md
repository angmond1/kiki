...
| **kk-mail** | 메일관리 | 자연어 메일검색, 자동 폴더분류, 불건전 학회/저널 메일 자동스팸 | Opus (분류, 스팸처리 Sonnet) |
| **kk-wiki** | 규정검색 | KIST 위키 규정·지침 자연어 검색, 인용 페이지 최신 확인 | Opus |
...
0. KIST 포탈에 로그인 한 후,
1. **kk-pay**: 세금계산서/카드결제내역 파일·폴더 경로를 알려주면서 "여기 폴더에 있는 결제건들 지급신청하자"
2. **kk-dining**: "이번달 카드결제내역 파악해서 회의비 지급신청하자"
3. **kk-inspect**: 계산서/영수증, 검수물품 사진 파일·폴더 경로를 알려주면서 "여기 폴더에 있는 결제건들 검수신청해줘"
4. **kk-budget**: "내가 참여하고 있는 과제에서 재료비 잔액 알려줘", "2EXXXXX 과제에서 김키키가 사용한 금액 파악해줘"
5. **kk-mail**: "2024년 김키키와 논문작성건으로 주고받은 메일 파악해서 내용 정리해줘", "시그마 알드리치에서 오는 메일은 시약 폴더를 만들어서 자동분류되게 해줘", "지난 1주일간 메일 내역 파악해서 불량 학회/저널에서 온 메일 스팸처리해줘"
6. **kk-wiki**: "출장 식비 한도 알려줘", "법인카드 회의비에 사전결재가 필요한지 규정 찾아줘", "외자 구매 절차 관련 페이지 다 모아줘"

⚠️ 최초 2-3회는 과정을 지켜봐주면서 LLM의 실수를 알려주기를 권장함.
