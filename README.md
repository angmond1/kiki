...
| **ki-dining** | 회의비 처리 — 카드 회의비 추출·사전결재 매칭·**회의록 엑셀 master**·**fam_0704_02 지급신청서 완전자동작성·임시저장·결재상신**(NEXACRO 부모탭 JS, 카드매핑→계정/비목 콜백→통장표기→적요→회의록 입력→사전결재 연동→저장→상신). hwp 양산은 옵션 | ✅ 사용 가능 |
...
## ki-dining 한눈에 (v2, 2026-06-05~)
- **부트스트랩(1회)**: 이름 → 카드책임자 → 참여과제 확인 → 사전결재 면제(**I·S·B·F·부서운영비**) 확인 → **저장 모드(엑셀만/엑셀+한글)** → "Dooray 드라이브 RPA 처리? 예/아니요" → (예면)토큰·행정원 폴더 → 환경 점검(Chrome MCP·openpyxl).
- **작업**: 카드(법인+연구비) 회의비 후보 추출 → 사전결재 매칭(목적·장소·시간 자동, 없으면 과제보고서로 제목·내용 산출) → 인원(⌈금액 식대+음료 합산 ÷ 5만⌉+1) → 회의시간(카드승인시간 USETIME 참고 융통성) → 참석자 → 회의내용(10만원↑) → **회의록 엑셀 9컬럼 작성**(처리일 `{yymmdd}_회의록.xlsx` 1파일) → **fam_0704_02 지급신청서 직접 자동작성**(NEXACRO 부모탭 JS 11단계) → 임시저장 → 결재상신(별도 gw 창은 사용자 직접).
- **fam_0704 자동작성 11단계**: DOC_CLS="G" frozen 회피 → 식비안내 팝업 닫기 → 카드매핑(`doSetDesp`) → 계정 필터 popBudgList → `doDecision()` opener 콜백 → 통장표기(`dpstDispNm` + `common_onkillfocus` 동기화) → 적요 → 회의록 팝업[사용구분 `rd_UseType="3"` + 사전결재 `button00` 연동 + 회의록 원본 복원] → `doSave` → `bt_save` 임시저장 → `bt_approval` 결재상신.
- **5건 묶기**: 한 상신에 행추가로 최대 5건(8건→5+3 분할). 같은날 식당+카페 = 동일 상신건 1행.
- **결재선 안내**(gw 별도 창): 계정책임자(`ds_rqstGrid.RDSBJEMPNM`) 자동 확인 → 발의자 본인이면 "그대로 상신하세요" / 다르면 "{계정책임자}님 결재선에 추가하세요".
- **증빙 jpg**: 식당·명확한 카페 = 카드전표 갈음 / 카페·마트·편의점·호텔·제과겸업 = `{yymmdd}_{거래처}.jpg` (반드시 jpg 변환).
- **저장 모드**: `xlsx_only` (기본, Claude 항상 엑셀만 조회) / `xlsx_and_hwp` (보관 선호자 별지1호 hwp 동봉, `pyhwpx`+보안팝업 Alt+N watcher).
- 인증: 통합정보 조회·fam_0704 자동작성=SSO 세션(좌표 0, NEXACRO 부모탭 JS) / 두레이 업로드(옵션)=개인 토큰(`kiki.env` 공유). 자세히: [`skills/ki-dining/SKILL.md`](skills/ki-dining/SKILL.md), [`fam_0704_automation.md`](skills/ki-dining/references/fam_0704_automation.md).
