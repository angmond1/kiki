...
| **kk-wiki** | 규정·담당자검색 | KIST 위키 규정·지침 자연어 검색(관련 조항 종합·원문 인용·출처·최신 확인), 부서별업무분장표 기반 담당자 안내 | Opus |
...
| skill | 이렇게 요청 |
|-------|------------|
| **kk-pay** | 세금계산서/카드결제내역 파일·폴더 경로를 알려주면서 "여기 폴더에 있는 결제건들 지급신청하자" |
| **kk-dining** | "이번달 카드결제내역 파악해서 회의비 지급신청하자" |
| **kk-inspect** | 계산서/영수증, 검수물품 사진 파일·폴더 경로를 알려주면서 "여기 폴더에 있는 결제건들 검수신청해줘" |
| **kk-budget** | "내가 참여하고 있는 과제에서 재료비 잔액 알려줘"<br>"2EXXXXX 과제에서 김키키가 사용한 금액 파악해줘" |
| **kk-mail** | "2024년 김키키와 논문작성건으로 주고받은 메일 파악해서 내용 정리해줘"<br>"시그마 알드리치에서 오는 메일은 시약 폴더를 만들어서 자동분류되게 해줘"<br>"지난 1주일간 메일 내역 파악해서 불량 학회/저널에서 온 메일 스팸처리해줘" |
| **kk-wiki** | "3천 연구장비 구매요구서 작성시 필요서류와 결제선은?" |
