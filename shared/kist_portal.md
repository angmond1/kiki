...
> kiki 형제 skill(ki-rpa·ki-inspect·ki-budget 등)이 KIST 통합정보(`p.kist.re.kr:8081`, NEXACRO 14)를
> 조회할 때 공통으로 쓰는 패턴. **화면 클릭·좌표 없이 backend 를 fetch** → 해상도·모니터 무관.
> ki-rpa 에서 2026-06-04 실증(카드내역·과제목록 fetch 성공). **새 화면도 이 방식으로 먼저 시도하라.**
...
- 일부 backend 는 화면 호출 순서/세션 상태에 의존해 **직접 fetch 가 빈 응답**(ki-rpa `chkPopup` 사례, 화면 Enter 로는 동작) → 그땐 좌표 fallback 또는 config 우회.
...
| 소액검수신청 | `mis.mcs::mcs_0003` (ki-inspect 빌드 시 캡처) |
| 예실대비표(예산) | `mis.bdg::bdg_2030` (ki-budget 빌드 시 캡처) |
| 과제 심의요청 | `mis.rdc::rdc_2700` |

> ⚠️ **저장·제출(쓰기)은 회사 정책상 Edge 권장**(Chrome 저장 실패 사례 있음). **조회(fetch)는 Chrome 무방.** 쓰기까지 fetch 로 되는지는 화면별로 캡처·검증 필요(미검증 시 좌표/Edge).
